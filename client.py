...
ui = {
    "background": ch_info,
    ## icon block ##
    "player_sprite": UiRenderer(player.sprite.src, tex=player.sprite.tex, pos=(10, 10),
                                pivot=(0, 0), parent=icon_block, relpos=(0, 0), size=(60, 60)),
    ## name block
    "player_name": UiTextRenderer(player.name, font=fonts["ChInfo.H1"], fore=(255, 255, 255), back=(0, 0, 0, 0),
                                  parent=name_block, pos=(5, 2), relpos=(0, 0.5), pivot=(0, 1)),
    "player_title": UiTextRenderer(player.sheet.title, font=fonts["ChInfo.H2"], fore=(255, 255, 255), back=(0, 0, 0, 0),
                                   parent=name_block, pos=(5, 0), relpos=(0, 0.5), pivot=(0, 0)),
    "player_level": UiTextRenderer(f"LVL {player.sheet.level}", font=fonts["ChInfo.H2"], fore=(255, 255, 255),
                                   back=(0, 0, 0, 0), parent=name_block, pos=(-10, 0), relpos=(1, 0.5), pivot=(1, 1)),
    "player_race": UiTextRenderer(player.sheet.race, font=fonts["ChInfo.H2"], fore=(255, 255, 255), back=(0, 0, 0, 0),
...
try:
    while hui:
        ...
        Utils.debug(f'Connected to: {server_info["name"]}@{ip}:{port} as {creds.split("+")[0]}, {ROOT=}')
        if debug:
